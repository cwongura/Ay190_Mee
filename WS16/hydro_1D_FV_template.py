...
def hlle(hyd):
    ...
    # right are qm[:,i+1]
    for i in range(1,hyd.n-2):
        fluxl[0,i] = hyd.qp[1,i]
        fluxl[1,i] = hyd.qp[1,i]*hyd.velp[i]+hyd.pressp[i]
        fluxl[2,i] = (hyd.qp[2,i]+hyd.pressp[i])*hyd.velp[i]

        fluxr[0,i] = hyd.qm[1,i]
        fluxr[1,i] = hyd.qm[1,i]*hyd.velm[i]+hyd.pressm[i]
        fluxr[2,i] = (hyd.qm[2,i]+hyd.pressm[i])*hyd.velm[i]
...
mpl.plot(hyd.x,hyd.vel,"b-")
mpl.plot(hyd.x,hyd.press,"g-")
...
while(t < tend):
    ...
    if(i % 10 == 0):
        ...
        mpl.clf()
        p1, = mpl.plot(hyd.x,hyd.rho,"r-")
        p2, = mpl.plot(hyd.x,hyd.vel,"b-")        
        p3, = mpl.plot(hyd.x,hyd.press,"g-")
        mpl.legend((p1,p2,p3),("Rho","Velocity","Press"),loc=(0.,0.),
                   frameon=False)
        timestring = "%5.3f" % (t)
        ax = mpl.gca()
