...
def get_accels(r,m,p,rho,av,h,n,neibs,nneibs):
    ...
    for i in range(n):
        for j in range(nneibs[i]):
            jj = neibs[i,j]
            rij = r[i]-r[jj]
            accels[i] -= m[jj]*((p[i]/rho[i]**2)*dsmoothing_kernel_rij(rij,h)+
            (p[jj]/rho[jj]**2)*dsmoothing_kernel_rij(rij,h)+av[i,jj]*
...
def get_energyRHS(r,m,p,rho,v,av,h,n,neibs,nneibs):
    ...
    for i in range(n):
        for j in range(nneibs[i]):
            ...
            rij = r[i]-r[jj]
            vij = v[i]-v[jj]
            epsrhs[i] += 0.5*m[jj]*((p[i]/rho[i]**2)+(p[jj]/rho[jj]**2)
            +av[i,jj])*vij*dsmoothing_kernel_rij(rij,h)
...
for nt in range(1,ntmax):
    ...
    if nt == 1 or nt % 5 == 0:
        ...
        cur_time = "time =" + str(time)
        mpl.text(0.2, 1., cur_time)
        mpl.draw()
